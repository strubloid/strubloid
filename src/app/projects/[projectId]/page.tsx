...
export default function ProjectDetailPage() {
  ...
  const latestChat = [...project.chats].sort(
        ...
        <div className="cw-shell">
          <header className="cw-section">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <Link href="/projects" className="cw-button">
                ← Workbench
              </Link>
            ...
            <p className="cw-subtitle">
              This is the durable context surface for the project. Every project chat can become
              memory fuel for Brain mode, so this page is optimized for continuing the right thread
              quickly.
            </p>
          </header>
          ...
          <section className="cw-section">
            <BentoGrid>
              <BentoCard span="featured" label="context state" title="Memory container status">
                <div className="cw-metric-row">
                  <div className="cw-metric">
                    <span className="cw-metric-value">{project.chatCount}</span>
                    <span className="cw-metric-label">chat threads</span>
                  </div>
                  <div className="cw-metric">
                    <span className="cw-metric-value">{totalMessages}</span>
                    <span className="cw-metric-label">messages</span>
                  </div>
                  <div className="cw-metric">
                    <span className="cw-metric-value">{project.isStarred ? 'ON' : 'OFF'}</span>
                    <span className="cw-metric-label">priority pin</span>
                  </div>
                </div>
              ...
              <BentoCard
                span="third"
                label="latest signal"
                title={latestChat ? latestChat.title : 'No signal yet'}
              >
                {latestChat ? (
                  <>
                    <p>
                      {latestChat.messages?.length ?? 0} messages · updated{' '}
                      {formatDate(latestChat.updatedAt)}
                    </p>
                    <div className="cw-actions">
                      <Button href={`/chat/${latestChat.id}`}>Resume latest</Button>
                    </div>
                  </>
                ) : (
              ...
              <BentoCard span="full" label="configuration" title="Project configuration">
                {isLoading ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 rounded border border-[var(--color-border)] bg-transparent px-2 py-1 text-[var(--color-text)] focus:ring-2 focus:ring-[var(--color-accent)] focus:outline-none"
                      />
                    </div>
                    <div className="flex items-center gap-3">
                  ...
                  <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
                    <div>
                      ...
                      <p className="cw-card-copy">
                        Choose a thread to continue the context, or create a fresh one when the
                        topic changes.
                      </p>
                    </div>
                    <Badge>
                      {project.chats.length} thread{project.chats.length !== 1 ? 's' : ''}
                    </Badge>
                  </div>
                        ...
                        <Link key={chat.id} href={`/chat/${chat.id}`} className="cw-list-item">
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="block text-xs text-[var(--cw-muted)]">
                              {chat.messages?.length} messages · updated{' '}
